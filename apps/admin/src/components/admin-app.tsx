...
function AiContentStudio({ setActive }: { setActive: (entity: Entity) => void }) {
  ...
  async function loadHistory() {
    try {
      const response = await fetch("/api/cms/ai/generations");
  ...
  async function run(action: "generate-outline" | "generate-meta" | "generate-article", createDraft = false) {
    ...
    try {
      const response = await fetch(`/api/cms/ai/${action}`, {
        method: "POST",
  ...
  async function generateImage() {
    ...
    let payload: Record<string, unknown>;
    try {
      const response = await fetch("/api/cms/ai/generate-image", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
...
function MenuBuilder({ roles }: { roles: string[] }) {
  ...
  async function load() {
    try {
      const [menusResponse, suggestionsResponse] = await Promise.all([
        fetch("/api/cms/menus"),
        fetch("/api/cms/menu-link-suggestions"),
      ]);
  ...
  async function createItem(parentId: number | null = null) {
    ...
    try {
      ...
      const response = await fetch("/api/cms/menu-items", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function saveItem(item: MenuItem) {
    if (!canWrite) return;
    try {
      const response = await fetch(`/api/cms/menu-items/${item.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          menuId: item.menuId,
  ...
  async function deleteItem(item: MenuItem) {
    ...
    if (!accepted) return;
    try {
      const response = await fetch(`/api/cms/menu-items/${item.id}`, { method: "DELETE" });
  ...
  async function saveOrder() {
    ...
    setSavingOrder(true);
    try {
      const response = await fetch(`/api/cms/menus/${activeMenu.id}/reorder`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
...
function EstimatorPanel({ roles }: { roles: string[] }) {
  ...
  async function load() {
    ...
    try {
      const [configResponse, estimatesResponse] = await Promise.all([
        fetch("/api/cms/construction-estimator/config"),
        fetch("/api/cms/construction-estimator/estimates?limit=8"),
      ]);
  ...
  async function runPreview() {
    ...
    const response = await fetch("/api/cms/construction-estimator/preview", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
  ...
  async function saveConfig() {
    ...
    const response = await fetch("/api/cms/construction-estimator/config", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
  ...
  async function resetDefaultConfig() {
    const response = await fetch("/api/cms/construction-estimator/reset-default", { method: "POST" });
    if (!response.ok) {
...
function EntityPanel({ entity, roles }: { entity: Entity; roles: string[] }) {
  ...
  async function load(page = 1) {
    try {
      ...
      const response = await fetch(`/api/cms/${entity}?${params}`);
      if (response.status === 401) {
        window.location.href = "/login";
        return;
      }
  ...
  async function loadScheduled(month = calendarDate) {
    ...
    try {
      const response = await fetch(`/api/cms/scheduled-posts?${params}`);
  ...
  async function loadProjectCategories() {
    try {
      const response = await fetch("/api/cms/project-categories?limit=200");
  ...
  async function loadFilterOptions() {
    try {
      const response = await fetch("/api/cms/project-filter-options?limit=500");
  ...
  async function submit(values: Record<string, unknown>) {
    ...
    if (entity === "posts" && values.status === "scheduled" && values.scheduledAt && saved?.id) {
      ...
      try {
        scheduleResponse = await fetch(`/api/cms/posts/${saved.id}/schedule`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async function remove(row: CmsItem) {
    ...
    let response: Response;
    try {
      response = await fetch(`/api/cms/${entity}/${row.id}`, { method: "DELETE" });
    } catch (error) {
  ...
  async function publishPost(row: CmsItem) {
    ...
    let response: Response;
    try {
      response = await fetch(`/api/cms/posts/${row.id}/publish`, { method: "POST" });
    } catch (error) {
  ...
  async function cancelSchedule(row: CmsItem) {
    ...
    let response: Response;
    try {
      response = await fetch(`/api/cms/posts/${row.id}/cancel-schedule`, { method: "POST" });
    } catch (error) {
...
function LeadDetailPanel({ lead, onBack, onUpdated }: { lead: CmsItem; onBack: () => void; onUpdated: () => Promise<void> }) {
  ...
  async function saveLead() {
    ...
    let response: Response;
    try {
      response = await fetch(`/api/cms/leads/${lead.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function addNote(event: React.FormEvent<HTMLFormElement>) {
    ...
    let response: Response;
    try {
      response = await fetch(`/api/cms/leads/${lead.id}/notes`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
