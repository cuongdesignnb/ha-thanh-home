...
const scopeIcons: LucideIcon[] = [UserRoundCheck, DraftingCompass, HardHat, Factory, Wrench, ClipboardCheck, ShieldCheck, BadgeCheck];
const processIcons: LucideIcon[] = [UserRoundCheck, DraftingCompass, Banknote, Factory, HardHat, Wrench, ClipboardCheck];
const whyIcons: LucideIcon[] = [Medal, Factory, ClipboardCheck, Clock3, ShieldCheck, Headphones];
const statIcons: LucideIcon[] = [TimerReset, Building2, Sparkles, Headphones];

type LandingType = ReturnType<typeof nhaXuongLandingWithDefaults>;

export default async function ThiCongNhaXuongPage() {
    ...
    <section className="xay-nha-hero" style={{ backgroundImage: `linear-gradient(90deg, rgba(7, 23, 18, 0.92) 0%, rgba(7, 23, 18, 0.72) 43%, rgba(7, 23, 18, 0.18) 78%), url(${landing.heroImageUrl})` }}>
      <div className="container">
        <nav className="xay-nha-breadcrumb" aria-label="Breadcrumb">
          <a href="/">Trang chủ</a><span>/</span><a href="/dich-vu">Dịch vụ</a><span>/</span><strong>Thi công nhà xưởng</strong>
        </nav>
        <div className="xay-nha-hero-content">
          ...
          <div className="xay-nha-actions">
            ...
            <a className="cta secondary" href="/lien-he?nguon=thi-cong-nha-xuong"><PhoneCall size={18} /> {landing.secondaryCtaLabel}</a>
          </div>
        </div>
      </div>
    </section>
    ...
    <section className="section cream">
      <div className="container">
        ...
        {projects.length ? (
          <div className="xay-nha-project-grid">
            {projects.map((project, index) => (
              <article className="xay-nha-project-card" key={project.id}>
                <div className="xay-nha-project-image" style={{ backgroundImage: `url(${thumbnailUrl(project, projectImages[index % projectImages.length])})` }}><span>{project.categoryRef?.name || project.category || "Nhà xưởng"}</span></div>
          ...
          <div className="xay-nha-empty">Chưa có dự án nhà xưởng đã xuất bản. Vui lòng thêm dự án trong admin để hiển thị tại đây.</div>
        )}
      </div>
    </section>
  );
    ...
    <section className="section">
      <div className="container xay-nha-final-cta">
        ...
        <div className="xay-nha-actions">
          ...
          <a className="cta" href="/lien-he?nguon=thi-cong-nha-xuong"><MessageCircle size={18} /> {landing.secondaryCtaLabel}</a>
        </div>
      </div>
    </section>
  );
