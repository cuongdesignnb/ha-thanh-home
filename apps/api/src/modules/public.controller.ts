...
export class PublicController {
  ...
  async services(@Query() query: Record<string, string>) {
    const { page, limit, skip } = parsePagination(query);
    const where = fixedServicePageWhere({
      status: ContentStatus.published,
  ...
  async service(@Param("slug") slug: string) {
    const data = await this.prisma.service.findFirst({ where: fixedServicePageWhere({ slug, status: ContentStatus.published }), include: { thumbnailMedia: true } });
